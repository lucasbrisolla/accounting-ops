...
Piloto de destilação editorial do livro *Cost Accounting Fundamentals* (Steven Bragg) para aprofundar o `accounting-ops`.
...
O modelo desta pasta segue o padrão editorial já testado nos outros livros de `books/`, com foco mais forte em custos, valuation, variâncias, ambiente industrial e conexão entre operação, estoque e margem.

## Estratégia

1. **Índice** (`cost-accounting-fundamentals-index.md`) — mapa de destilação editorial do livro.
...
> "Uma pessoa de accounting, controladoria ou FP&A usaria isso para entender custo, analisar margem, explicar variâncias ou conectar operação com resultado sem reescrever do zero?"
...
Este livro tende a alimentar mais fortemente `concepts`, `workflows`, `playbooks` e alguns processos ligados a estoque, valuation, process costing, standard costing e perdas operacionais.
